...
import { pickWorkTitle } from '../../lib/utils.ts';
...
export default function WorkFormModal({ 
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingWork) {
        ...
        setTitleKn(editingWork.titleKn || '');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      title: finalTitle,
      titleKn: titleKn || '',
      description: 'General Task Details',
      descriptionKn: '',
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" style={{ zIndex: 60 }}>
      <div
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-xl w-full p-8 shadow-2xl relative animate-slide-up max-h-[88vh] overflow-y-auto"
      ...
      >
        ...
        <h3 className="text-md font-display font-extrabold text-slate-900 dark:text-white mb-6">
          {editingWork ? t(language, 'editWorkPlace') : t(language, 'addWorkPlace')}
        </h3>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div>
            ...
            {(() => {
              ...
              const selectOptions = [
                ...allPlaces.map((p) => {
                  const work = allWorks.find((item) => item.title === p);
                  return { value: p, label: work ? pickWorkTitle(language, work) : p };
                }),
                { value: 'custom', label: t(language, 'addCustomWorkPlace') },
              ];
              return (
                <CustomSelect
                  value={title}
                  ...
                  placeholder={t(language, 'selectWorkPlace')}
                />
              );
            })()}
          </div>
          ...
          {title === 'custom' && (
            <div className="animate-scale-in">
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">
                {t(language, 'newWorkPlaceName')} *
              </label>
              <input
                type="text"
                required
                placeholder={t(language, 'enterCustomWorkPlaceName')}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">
              {t(language, 'workPlaceKn')}
            </label>
            <input
              type="text"
              placeholder={t(language, 'enterWorkPlaceKn')}
              value={titleKn}
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-slate-100 dark:border-slate-800/80 mt-4">
            ...
            >
              ...
              {editingWork ? t(language, 'saveChanges') : t(language, 'addTask')}
            </button>
          </div>
        </form>
      </div>
